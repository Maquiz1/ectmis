...
if ($user->isLoggedIn()) {
    if (Input::exists('post')) {
        ...
        if ($_GET['id'] == 11) {
            $data = null;
            $filename = null;
            if (Input::get('sufficient')) {
                $data = $override->get4('batch', 'amount', 'notify_amount', 'status', 1);
                $filename = 'SUFFICIENT';
            } elseif (Input::get('running_low')) {
                $data = $override->get5('batch', 'amount', 'notify_amount', 'status', 1);
                $filename = 'RUNNING LOW';
            } elseif (Input::get('out_stock')) {
                $data = $override->get6('batch', 'amount', 'status', 1);
                $filename = 'Out of Stock';
            } elseif (Input::get('expired')) {
                $data = $override->get7('batch', 'expire_date', date('Y-m-d'), 'status', 1);
